            ...
            <v-layout align-center>
                ...
                <v-flex md6 class='review__note'>
                    <template v-for='num in [5,4,3,2,1]'>
                        <input type="radio" :key='num' :id='num' name="rating" :value='num' v-model='bookNote'>
                        <label :for='num' :key='num + 1'> 
                            {{num}} Stars
                        </label>
                    </template>
                </v-flex>
                ...
                <v-flex xs12 >
                    <v-layout row justify-center>
                        <!-- Bouton d'edit trigger la méthode formSubmit, bouton disabled si pas au moins de titre -->
            ...
            formSubmit() {
                ...
                if (this.bookTitle !== '') {
                    if (this.submitTxt === 'Ajouter') {
                        ...
                        this.$emit('editEvent',{book, index: this.idxSelected})  // Envoi à Table
                        eventBus.$emit('editDone')                               // Envoi à Container
                    }
...
.review__note:not(:checked) > input {
    position:absolute;
    top:-9999px;
}
.review__note:not(:checked) > label {
    float:right;
    width:1em;
    overflow:hidden;
    white-space:nowrap;
...
.review__note:not(:checked) > label:before {
    content: '★ ';
}
.review__note > input:checked ~ label {
    color: #ffc700;    
}
.review__note:not(:checked) > label:hover,
.review__note:not(:checked) > label:hover ~ label {
    color: #deb217;  
}
.review__note > input:checked + label:hover,
.review__note > input:checked + label:hover ~ label,
.review__note > input:checked ~ label:hover,
.review__note > input:checked ~ label:hover ~ label,
.review__note > label:hover ~ input:checked ~ label {
    color: #c59b08;
}
    /* .review__note {
    display: -ms-flexbox;
